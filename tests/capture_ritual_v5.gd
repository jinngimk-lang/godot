...
func _run() -> void:
	...
	var support := scene.get_node("LeftHand") as Node3D
	...
	if float(crumple.get_progress()) < 0.72 or ritual.get_phase_name() != "RITUAL_COMPLETE":
		...
		quit(1)
		return
	var support_delta := support.position - support_home
	if support_delta.x > -0.02:
		push_error("CAPTURE_V5: support hand failed to visibly stage inward; delta=%s" % str(support_delta))
		quit(1)
		return
	print("CAPTURE_V5: phase=%s crumple=%.3f lid_y=%.4f support_delta=%s" % [ritual.get_phase_name(), float(crumple.get_progress()), scene.get_node("Lid").position.y, str(support_delta)])
	scene.queue_free()
	await process_frame
	quit(0)
