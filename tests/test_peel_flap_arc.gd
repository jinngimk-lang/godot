...
func run() -> Array[String]:
	...
	# Object-only north star: the free flap must visibly bow away from the chord,
	# but the printed sheet remains broad/readable instead of becoming the old
	# ribbon-like curl that was necessary to sell a hand pinch animation.
	var width := 1.25
	var radius := 0.53
	var label_y := 0.22
	var offset := 0.018
	...
	if arc_ratio<0.075:
		failures.append("FLAP_ARC_RED: lifted flap is too chord-flat to reveal paper depth; ratio=%.4f" % arc_ratio)
	if arc_ratio>0.18:
		failures.append("FLAP_ARC_RED: lifted flap is too ribbon-curled for the approved readable-sheet reference; ratio=%.4f" % arc_ratio)

	var nominal := width/float(segments)
	for i in range(points.size()-1):
		var spacing := points[i].distance_to(points[i+1])
		if spacing>nominal*1.8:
			failures.append("FLAP_ARC_RED: curvature stretched segment %d to %.5f" % [i,spacing])
			break
	return failures
