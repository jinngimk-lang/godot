...
func _run() -> void:
	...
	# Enter the real post-peel ritual without changing production state directly.
	...
	if not is_equal_approx(float(crumple.get_progress()), after_first):
		push_error("REGRAB_RED: releasing a squeeze must preserve accumulated crumple progress")
		quit(1)
		return

	# Owner requirement: the cup should remain a tactile toy. A fresh second press
	# after release must reacquire crumple gesture ownership and add another dent.
