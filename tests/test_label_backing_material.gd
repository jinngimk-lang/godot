...
func run() -> Array[String]:
	...
	# Runtime evidence showed the old 150-degree roll splitting the label into
	# several diagonal ribbons. The supplied photography keeps the peeled sheet
	# readable as one broad panel with a restrained curl at the free edge.
	var p := 0.52
	...
	if free_roll < deg_to_rad(22.0) or free_roll > deg_to_rad(55.0):
		failures.append("LABEL_BACKING_RED: free peel curl must stay within a photographic 22-55 degree band; got %.1f°" % rad_to_deg(free_roll))
	if mid_roll <= deg_to_rad(6.0) or mid_roll >= free_roll:
		failures.append("LABEL_BACKING_RED: flap curl should decay smoothly through the loose span")
	...
	if not bool(visual.has_distinct_peeled_backing()):
		failures.append("LABEL_BACKING_RED: a partially peeled fibrous bar label must build a separate visible underside surface")
	var surface_count: int = visual.mesh.get_surface_count() if visual.mesh != null else 0
	if surface_count < 4:
		failures.append("LABEL_BACKING_RED: partial fibrous peel needs front + backing + glue/edge surfaces; got %d" % surface_count)
