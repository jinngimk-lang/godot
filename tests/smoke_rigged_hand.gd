...
func _run() -> void:
	...
	var hand_script: Script = load("res://scripts/hands/hand_visual.gd") as Script
	if hand_script == null:
		failures.append("HandVisual script did not load")
	else:
		var right_hand: Node3D = hand_script.new() as Node3D
		if right_hand == null:
			failures.append("HandVisual did not instantiate as Node3D")
		else:
			right_hand.name = "RuntimeHand"
			root.add_child(right_hand)
			right_hand.call("setup", true)
			await process_frame
			await process_frame
			var rigged_root: Node = right_hand.find_child("RiggedHand", true, false)
			if rigged_root == null:
