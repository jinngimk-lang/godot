...
func _log_variant(scene: Node, label: String) -> void:
	...
	var ice_positions: Array[String] = []
	var container: Node = contents.get_node_or_null("IceContents")
	if container != null:
		for child in container.get_children():
			if child is MeshInstance3D:
				var p := (child as MeshInstance3D).position
				ice_positions.append("(%.3f,%.3f,%.3f)" % [p.x, p.y, p.z])
	print("CAPTURE_V6: %s id=%s dims=%.3f/%.3f/%.3f lid_visible=%s cap_top=%s contents=%d ice_local=%s" % [
		label,
