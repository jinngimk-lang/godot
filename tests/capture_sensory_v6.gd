...
func _run() -> void:
	...
	# Unlock and rotate to the second tactile set.
	...
	# Unlock all three and rotate from silky_long to crisp_seal.
	session.record_ritual_complete()
	session.record_ritual_complete()
	session.record_ritual_complete()
	session.advance_item()
	...
	# Drive the real post-peel/crumple route for one visual mid-fold frame.
...
func _log_variant(scene: Node, label: String) -> void:
	...
	print("CAPTURE_V6: %s id=%s dims=%.3f/%.3f/%.3f lid_visible=%s lid_y=%.3f contents=%d ice_global_y=%s" % [
		label,
