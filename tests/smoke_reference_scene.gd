...
func _run() -> void:
	...
	for path in ["Camera","Cup","Lid","PeelLabel","LabelPrint","PointerAdapter","PeelAudio","HUD","VenuePresentation","ProductPresentation","ResidueVisual","GuidedJourneyPresentation","ReferenceBackdrop","CornerPeelPresentation","CursorPresentation"]:
	...
	var corner := scene.get_node("CornerPeelPresentation") as CornerPeelPresentation
	...
	if rail == null or not rail.visible:
		_fail("five-scene JourneyRail must be visible immediately",scene); return
	if edge.visible:
		_fail("legacy gold hotspot must remain hidden",scene); return
	if label.visible:
		_fail("hand-era LabelVisual ribbon must be hidden behind localized CornerPeelPresentation",scene); return
	var corner_mesh := corner.get_node_or_null("CornerPeelLabel") as MeshInstance3D
	if corner_mesh == null or corner_mesh.mesh == null or not corner_mesh.visible:
		_fail("localized corner-peel mesh must be the visible label authority",scene); return
	...
	if environment == null or environment.environment == null:
		_fail("reference venue environment missing",scene); return
	...
	for hud_path in ["HUD/ProgressPanel","HUD/ControlsPanel","HUD/HowToPanel","HUD/JourneyRail"]:
		if scene.get_node_or_null(hud_path) == null:
			_fail("approved HUD missing %s" % hud_path,scene); return
	...
	for i in range(EXPECTED.size()):
		...
		if button == null:
			_fail("JourneyRail missing scene %d button" % (i+1),scene); return
		...
		if venue.get_active_profile_id()!=expected_scene:
			_fail("scene %d must activate venue %s" % [i+1,expected_scene],scene); return
		if product.get_active_kind()!=expected_kind:
			_fail("scene %d must activate product %s" % [i+1,expected_kind],scene); return
		if guide.get_active_scene_index()!=i:
			_fail("scene rail must highlight index %d" % i,scene); return
		if label.visible:
			_fail("scene %d must keep hidden simulation label out of the final render" % (i+1),scene); return
		corner_mesh = corner.get_node_or_null("CornerPeelLabel") as MeshInstance3D
		if corner_mesh==null or corner_mesh.mesh==null or not corner_mesh.visible:
			_fail("scene %d must restore localized visible label" % (i+1),scene); return
		if product.get_node_or_null(expected_node)==null:
			_fail("scene %d missing hero semantic node %s" % [i+1,expected_node],scene); return
		var paper := expected_kind=="paper_cup"
		if cup.visible!=paper or lid.visible!=paper:
			_fail("base interaction cup/lid visibility must only be owned by Coffee Shop",scene); return

	print("PASS: persistent five-scene object-only journey with localized corner-peel authority")
	scene.queue_free()
	await process_frame
	quit(0)
...
func _collect_text(node: Node) -> String:
	var output := ""
	if node is Label: output += (node as Label).text+"\n"
	if node is Button: output += (node as Button).text+"\n"
	for child in node.get_children(): output += _collect_text(child)
	return output
