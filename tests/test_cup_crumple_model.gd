...
func run() -> Array[String]:
	...
	# CHALLENGER: equivalent physical squeeze must be approximately invariant
	# to OS/input sampling. A 20px continuous inward move may arrive as one
	# 20px mouse event or twenty 1px high-rate touch/motion events. Rigidity is
	# a gesture threshold; charging the deadzone once per packet makes the same
	# hand motion feel radically different across devices and frame rates.
