...
func run() -> Array[String]:
	...
	for variant in session.VARIANTS:
		...
		# Representative clean sustained pull. This is intentionally below each
		# scene's tear-speed threshold; the contract is about tactile duration,
		# not punishing the player with damage.
		...
		# Do not solve the first failure by making the label feel frozen. Another
		# two seconds of the same clean pull must still finish the peel.
