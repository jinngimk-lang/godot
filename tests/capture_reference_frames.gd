...
func _run() -> void:
	...
	for capture_case in CASES:
		...
		if cursor != null:
			cursor.set_debug_position(Vector2(640,360))
...
func _stage_direct_peel(scene: Node, capture_case: Dictionary) -> bool:
	...
	if label == null or residue == null or camera == null or cursor == null or session == null:
		push_error("CAPTURE_RED: direct peel staging is missing label/residue/camera/cursor/session")
		quit(1); return false
	var progress := float(capture_case["progress"])
	var front := label.get_front_position(progress)
	var desired_grip_local := front+Vector3(-0.54,0.06,0.28)
	...
	cursor.set_debug_position(camera.unproject_position(label.to_global(grip_local)))
...
func _assert_direct_peel(scene: Node, capture_case: Dictionary) -> bool:
	...
	if label == null or not label.visible:
		push_error("CAPTURE_RED: %s lost the live label during settle" % String(capture_case["peel"])); quit(1); return false
