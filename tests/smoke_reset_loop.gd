...
func _run() -> void:
	...
	if label == null or corner == null or hud == null or cup == null or lid == null or venue == null or product == null or residue == null or session == null:
	...
	for key in expected_controls.keys():
		if String(controls.get(key,""))!=String(expected_controls[key]):
			_fail("control %s must map to %s" % [key,expected_controls[key]],scene); return
	...
	for i in range(5):
		...
		if label.visible:
			_fail("scene %d must keep hand-era simulation ribbon hidden" % (i+1),scene); return
		var visible_label := corner.get_node_or_null("CornerPeelLabel") as MeshInstance3D
		if visible_label==null or visible_label.mesh==null or not visible_label.visible:
			_fail("scene %d reset must restore localized visible label" % (i+1),scene); return
		var paper := i==0
	...
	for _f in range(2): await process_frame
	...
	if label.visible:
		_fail("R must not re-show the hidden hand-era ribbon",scene); return
	var reset_visual := corner.get_node_or_null("CornerPeelLabel") as MeshInstance3D
