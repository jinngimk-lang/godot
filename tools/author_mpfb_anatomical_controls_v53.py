...
def _flex_sign(arm, bone_name: str, axis_local: Vector, vessel_normal_local: Vector) -> float:
    ...
    # Establish a semantic positive-flex convention once from the rig: flexion bends
    # a phalanx toward the vessel-facing/palmar normal. This is not an endpoint solve.
...
def _apply_thumb_controls(arm, forward: Vector, span: Vector, normal: Vector) -> dict:
    ...
    current.normalize()
    # A support-grip thumb should move across the near/upper palm rather than mirror the
    # four fingers. The desired direction is expressed only in palm semantic axes.
    ...
    # Pick the bounded opposition direction that points the base closer to the semantic
    # desired vector; this establishes control sign without copying a reference transform.
    ...
    # Distal thumb curl uses the palm span as a stable anatomical flexion axis, but with
    # its own sign and much smaller magnitudes than the fingers.
...
def run() -> None:
    ...
    # Resolve normal sign against the actual product-facing direction after whole-hand
    # placement while keeping the control frame on the GameEngine rig itself.
