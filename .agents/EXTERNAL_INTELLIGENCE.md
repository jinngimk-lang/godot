...
Source: upstream `godotengine/godot` issue #122791, opened 2026-08-25, and fix PR #122825, opened 2026-08-26 and merged 2026-08-27.

Finding: the reporter reproduces severe freezing while dragging on an actual Windows touchscreen in Godot 4.7.2 stable and 4.8 dev, while 4.7.1 does not reproduce. The issue is labeled `confirmed` in addition to `bug`, `regression`, `performance`, and `platform:windows`. On 2026-08-26 a Godot maintainer independently reproduced substantial lag after roughly 35–40 seconds of dragging on a 60 Hz Windows touchscreen, did not reproduce it with multiple pen tablets, and did not reproduce it when the same touch digitizer was mapped to a 120 Hz display. Upstream PR #122825, `[Windows] Process all WM_(NC)MOUSEMOVE messages when touch screen/pen input is detected`, was then validated by the original reporter and **merged to `master` on 2026-08-27** as merge commit `193c91493e8be7d8bf004bd6e70e1fe155669491`; issue #122791 closed as completed at the same time. The merged PR now carries the upstream `cherrypick:4.7` label, meaning it is explicitly being considered for a future 4.7.x maintenance release even though its milestone remains 4.8.

Relevance to Peel Calm: the production interaction target is PC mouse first, so this is still not evidence to roll back the 4.7.2 mouse fix. Merge plus reporter validation makes the upstream mitigation substantially stronger evidence than the former open-candidate state. The new `cherrypick:4.7` label also removes the prior assumption that 4.8 stable is necessarily the earliest possible fixed release line. However, consideration for cherry-pick is not the same as a completed backport or a released binary. Semantic mouse/touch ownership tests still cannot stand in for actual Windows device/performance validation.

Decision: keep Godot 4.7.2 as production and do not broaden touch support or claim Windows touchscreen readiness on 4.7.2. Track the merged fix for an actual 4.7 branch cherry-pick/release and for 4.8 stable. If a future 4.7.x stable release contains the fix, evaluate that maintenance release promptly rather than waiting for 4.8; otherwise 4.8 remains the known merged upstream line. Only treat the risk as mitigated after a stable build containing the fix is available and Peel Calm either passes isolated real-touch validation or has equivalent device evidence. Do not vendor or self-maintain the engine patch in this repository.

Status: WATCH — upstream fix merged and explicitly considered for 4.7 cherry-pick; production stays 4.7.2 until a stable containing release plus touch validation exists.
...
- 2026-08-28 — upstream PR #122825 merged (`193c9149...`), #122791 closed completed, and the fix gained `cherrypick:4.7`; removed the stale assumption that only 4.8 can be the earliest fixed stable line while retaining the stable-release plus real-touch gate.
