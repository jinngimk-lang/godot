...
func _init(config: Dictionary = {}) -> void:
	_base_adhesion = maxf(_finite_or(float(config.get("base_adhesion", 12.0)), 12.0), 0.001)
	...
	_bond_response = clampf(_finite_or(float(config.get("bond_response", 10.0)), 10.0), 0.1, 30.0)
