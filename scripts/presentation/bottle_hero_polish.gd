...
func build_preview_for_kind(kind: String) -> void:
	...
	_build_metal_cap()
	_build_neck_ring()
	_build_highlight("BottleHighlightLeft",-0.175,0.23,0.88,HIGHLIGHT_ALPHA)
	_build_highlight("BottleHighlightRight",0.205,0.12,0.66,HIGHLIGHT_ALPHA*0.68)
...
func _build_metal_cap() -> void:
	...
	cap.material_override = _metal(Color(0.78,0.80,0.80),0.23)
	add_child(cap)
	for i in range(3):
		...
		ridge.material_override = _metal(Color(0.84,0.85,0.84),0.18)
		add_child(ridge)
...
func _build_neck_ring() -> void:
	...
	material.albedo_color = Color(0.94,0.98,0.98,0.28)
	material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	material.roughness = 0.045
	material.metallic_specular = 0.92
...
func _build_highlight(node_name: String, x: float, y: float, height: float, alpha: float) -> void:
	...
	quad.size = Vector2(0.048,height)
	highlight.mesh = quad
	highlight.position = Vector3(x,y,0.334)
	var material := StandardMaterial3D.new()
...
func _metal(color: Color, roughness: float) -> StandardMaterial3D:
	...
	material.metallic_specular = 0.80
	return material
