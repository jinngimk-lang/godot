...
func _build_for_hand(hand_name: String, dynamic_hand: bool) -> void:
	...
	# Preserve the existing thin WristCuff at the skin/fabric seam, but retire
	# the old straight procedural cone. The replacement below is one connected
	# curve mesh, so there is no rigid cylinder/sphere elbow joint to reveal.
	...
	# from both wrists, authored -X exits screen-left for the dynamic right hand,
	# and authored +X exits screen-right for the support left hand.
	var start_authored := Vector3(0.0, 0.0, 0.020)
	var control_authored := Vector3(0.055 * side, 0.0, 0.285 if dynamic_hand else 0.305)
	var end_authored := Vector3(0.220 * side, 0.0, 0.715 if dynamic_hand else 0.750)
...
func _build_curve_mesh(start: Vector3, control: Vector3, end: Vector3) -> ArrayMesh:
	...
	for ring_index in range(CURVE_RINGS):
		...
		# The measured authored paths live mostly in local X/Z, so local Y is a
		# stable cloth-height reference. Build an orthonormal ring frame from it;
		# this avoids the twist/discontinuity of separately oriented cylinders.
		...
		# A wrist-sized opening grows gradually toward the unseen forearm. Keep the
		# cross-section slightly oval so it reads as soft cloth around anatomy, not
		# as a perfect pipe.
		var ease := pow(t, 0.82)
		var radius := lerpf(0.061, 0.108, ease)
		var oval_height := lerpf(0.78, 0.84, t)
		for side_index in range(RING_SIDES):
			var angle := TAU * float(side_index) / float(RING_SIDES)
			var radial := ring_x * cos(angle) * radius + ring_y * sin(angle) * radius * oval_height
			vertices.append(point + radial)
			normals.append((ring_x * cos(angle) + ring_y * sin(angle) / oval_height).normalized())
